...
export const handleDiseaseDetect: RequestHandler = async (req, res) => {
  try {
    ...
    const isSvg = (() => {
      try { const txt = buffer.toString('utf8', 0, Math.min(200, buffer.length)).toLowerCase(); return txt.includes('<svg') || detectedMime === 'image/svg+xml'; } catch(e) { return false; }
    })();
    ...
    if (!(global as any)._detectionCache) (global as any)._detectionCache = new Map<string, { value: any; expires: number }>();
    const cache: Map<string, { value: any; expires: number }> = (global as any)._detectionCache;

    const crypto = await import('crypto');
    const hfModel = process.env.HUGGINGFACE_MODEL || 'nateraw/plant-disease-classification';
    const key = crypto.createHash('sha256').update(usedBuffer).update(hfModel).digest('hex');
    ...
    if (!hfKey) return res.status(500).json({ error: 'Server misconfiguration: missing HUGGINGFACE_API_KEY' });

    const fallbackModels = ['microsoft/resnet-50', 'google/vit-base-patch16-224'];
    const modelsToTry = [hfModel, ...fallbackModels.filter(m => m !== hfModel)];
    ...
    for (const model of modelsToTry) {
      ...
      while (attempt < 3) {
        ...
        try {
          ...
          const json = await resp.json().catch((e) => { console.error('[disease] failed to parse HF response as JSON', e); return null; });

          const output = Array.isArray(json) && json.length ? { label: json[0].label, score: json[0].score, all: json, model } : { raw: json, model };

          // store in cache with TTL
          cache.set(key, { value: output, expires: Date.now() + CACHE_TTL });
          if (cache.size > CACHE_MAX) { const it = cache.keys(); const first = it.next().value; if (first) cache.delete(first); }
