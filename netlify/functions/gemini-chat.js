...
exports.handler = async function (event, context) {
  try {
    ...
    if (!apiKey) {
      console.error("[netlify/gemini-chat] Error: API key not found in environment variables");
      console.error("[netlify/gemini-chat] Available env vars:", Object.keys(process.env).filter(k => k.includes('GOOGLE') || k.includes('GEMINI') || k.includes('API')));
      return {
        statusCode: 500,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: "Server misconfiguration: missing GOOGLE_API_KEY or GEMINI_API_KEY",
